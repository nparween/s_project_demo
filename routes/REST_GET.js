...
// student/api/get/?c={target_column}&q={target_value}&order={orderby}
exports.findByColumn = function (req,res) {
	...
	connection.query('SELECT * from student where ?? = ? ORDER BY ?? DESC LIMIT ? OFFSET ?',
...
// Get all records
// student/api/get/all?order={orderby}
exports.getAllRecords = function (req,res) {
	var connection = require('../model/dbconnection');
	var val = req.query.q;
	console.log("val...",val)
	console.log("ss",req.query.order)
	// If order not speficied, then use order date
	if (typeof req.query.order == 'undefined')
	{
		console.log("ss",req.query.order)
	...
	connection.query('SELECT * from student ORDER BY ?? DESC LIMIT ? OFFSET ?',
...
// student/api/get/search/?c={target_column}&s={start}&e={end}&order={orderby}
exports.rangeSearch = function (req,res) {
	...
	connection.query('SELECT * from student WHERE ?? > ? AND ?? < ? ORDER BY ?? DESC LIMIT ? OFFSET ?',
